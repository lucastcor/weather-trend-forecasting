...
OUTPUT_RESULTS_PATH = Path("reports/hgb_results.csv")
OUTPUT_IMPORTANCE_PATH = Path("reports/hgb_feature_importance.csv")
OUTPUT_PREDICTIONS_PATH = Path("reports/hgb_predictions.parquet")
...
def build_pipeline() -> Pipeline:
    """Build the untuned HistGradientBoosting pipeline used for feature-set comparison."""
    return Pipeline(
        steps=[
...
def train_and_evaluate_feature_set(
    ...
    """Train and evaluate the untuned HistGradientBoosting model for one feature block."""
        ...
        [
            {
                ...
                "n_features": len(feature_columns),
                "model": "HistGradientBoostingRegressor_tuned",
                "mae": metrics["mae"],
                "rmse": metrics["rmse"],
                "r2": metrics["r2"],
                "mape": metrics["mape"],
